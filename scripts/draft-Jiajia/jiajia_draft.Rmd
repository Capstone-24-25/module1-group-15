...
#### Step 2: Selection of Top Predictive Proteins (Using a Larger Set)
```{r}
# Load necessary library
library(MASS)

# Define formula with all protein features
formula <- as.formula(paste("Group ~", paste(colnames(training_data)[-c(1,2)], collapse = " + ")))

# Run logistic regression with forward stepwise selection based on AIC
logistic_model <- glm(formula, data = training_data, family = binomial)
stepwise_model <- stepAIC(logistic_model, direction = "forward")

# Extract selected protein features
top_proteins_logreg <- names(coef(stepwise_model))[-1]  # Exclude intercept
print("Top proteins from logistic regression:")
print(top_proteins_logreg)
