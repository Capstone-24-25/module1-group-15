...
group_column <- training_data$group # For split group
protein_columns <- training_data %>% select(-group, -ados)  # only protein for testing
...
biomarker_long <- biomarker_clean %>%
  pivot_longer(cols = -c(group, ados),  # 除了 group 和 ados 列，其他列都转为长格式
               names_to = "protein",  # 新列名称，用于存放蛋白质名称
               values_to = "level")   # 新列名称，用于存放蛋白质水平
...
predictors <- training_data %>% select(-group, -ados) # only proteins
response <- training_data$group %>% factor()  # For split group
...
rf_top20 <- important_proteins_rf
ttest_top20 <- top_proteins_ttest
logreg_top20 <- top_20_proteins


all_top_proteins <- list(rf = rf_top20, ttest = ttest_top20, logreg = logreg_top20)


protein_counts <- unlist(all_top_proteins) %>%
  table() %>%
  as.data.frame()


fuzzy_intersection_proteins <- protein_counts %>%
  filter(Freq >= 2) %>%
  pull(.)
